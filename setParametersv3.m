function [param] = setParametersv3()
%General Parameters
%scan range
...
param.opticCentreP = [50,-50]; %in pixels
...
%Object coordinates
param.x = (-param.nPx/2:param.nPx/2-1)+param.opticCentreP(1); % number of voxels
param.y = (-param.nPx/2:param.nPx/2-1);
param.z = (-param.nPx/2:param.nPx/2-1)-param.opticCentreP(2);

%real scales
param.rx = param.x.*param.pxSz./param.mtz0-param.opticCentre(1);
param.ry = param.y.*param.pxSz./param.mtz0;
param.rz = param.z.*param.pxSz./param.mtz0+param.opticCentre(2);

%The real detector panel pixel density (number of pixels)
param.u =  param.x - param.opticCentreP(1);		% number of pixels
param.v =  param.z;
